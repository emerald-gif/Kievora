...
FOLLOW-UP CHIPS: At the end of substantive responses (not short replies, not responses ending in a question), include 2–3 short follow-up suggestions tied to the market topic just discussed. Format each at the very end: [FU]suggestion text[/FU]. Never use on greetings, short replies, or when ending with a question.`,
  },
...
FOLLOW-UP CHIPS: When you give a substantive answer (not a greeting, not a clarifying question), end with 1–2 follow-up suggestions as tight as your answers: [FU]suggestion text[/FU]. Skip on greetings or when your reply ends with a question.`,
  },
...
FOLLOW-UP CHIPS: At the end of substantive responses, include 2–3 bold follow-up suggestions tied to what was just discussed: [FU]suggestion text[/FU]. Skip on greetings, short replies, or when ending with a question.`,
  },
};
...
app.post('/api/kie', authenticate, async (req, res) => {
  ...
  if (wantsSearch) {
    ...
    if (searchConfigured) {
      ...
      sendSSE({ t: 'searchdone', count: searchResults ? searchResults.length : 0 });
  ...
  try {
    ...
    sendSSE({ t: 'done', model: effectiveModel, mode: effectiveMode, fallback: false, modeSuggestion });
    res.end();
