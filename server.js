...
// ─── Monthly KIE chat counter + topup balance ─────────────────────────────────
// Tracks how many KIE chat messages the user has sent this calendar month.
// Topup messages (purchased separately via Paystack) are stored in
// usage.kieTopupRemaining and are drawn down AFTER the monthly allocation
// runs out. Any unused topup messages expire at the end of the month when
// the monthly counter resets — this is stated clearly to users at purchase.
async function checkAndIncrementKieUsage(uid, planKey) {
  ...
  const sameMonth    = usage.kieResetMonth === month;
  const currentCount = sameMonth ? (usage.kieCount || 0) : 0;
  const topupLeft    = sameMonth ? (usage.kieTopupRemaining || 0) : 0;

  // Within monthly allocation
  if (currentCount < limit) {
    const newCount = currentCount + 1;
    await ref.set({ usage: { kieCount: newCount, kieResetMonth: month, kieTopupRemaining: topupLeft } }, { merge: true });
    return { allowed: true, remaining: limit - newCount + topupLeft, limit, count: newCount, fromTopup: false };
  }

  // Monthly allocation exhausted — draw from topup if available
  if (topupLeft > 0) {
    const newTopup = topupLeft - 1;
    await ref.set({ usage: { kieCount: currentCount, kieResetMonth: month, kieTopupRemaining: newTopup } }, { merge: true });
    return { allowed: true, remaining: newTopup, limit, count: currentCount, fromTopup: true, topupRemaining: newTopup };
  }

  // Hard stop — no allocation and no topup
...
app.post('/api/billing/webhook', express.raw({ type: '*/*' }), async (req, res) => {
  try {
    ...
    if (event.event === 'charge.success') {
      const { uid, plan, type, topupMessages } = event.data.metadata || {};
      if (!uid) { console.error('Paystack webhook: missing uid in metadata'); return res.sendStatus(200); }

      if (type === 'topup') {
        // Top-up: add messages to this month's balance, don't change the plan
        const month = new Date().toISOString().slice(0, 7);
        const uRef  = db.collection(USERS).doc(uid);
        const snap  = await uRef.get();
        const usage = (snap.exists && snap.data().usage) || {};
        const sameMonth = usage.kieResetMonth === month;
        const existing  = sameMonth ? (usage.kieTopupRemaining || 0) : 0;
        const toAdd     = Number(topupMessages) || 100;
        await uRef.set({
          usage: {
            kieTopupRemaining: existing + toAdd,
            kieResetMonth: month,
            kieCount: sameMonth ? (usage.kieCount || 0) : 0,
          },
        }, { merge: true });
        console.log(`💳 Paystack topup: ${uid} +${toAdd} messages (ref ${event.data.reference})`);
      } else if (plan && PLANS[plan]) {
        // Plan upgrade / change
        await db.collection(USERS).doc(uid).set({
          plan,
          planUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),
          usage: { kieCount: 0, kieResetMonth: new Date().toISOString().slice(0, 7), kieTopupRemaining: 0 },
        }, { merge: true });
        console.log(`💳 Paystack plan upgrade: ${uid} → ${plan} (ref ${event.data.reference})`);
      } else {
        console.error('Paystack webhook: unrecognized event metadata', event.data.metadata);
      }
    }
    res.sendStatus(200);
  } catch (e) {
...
app.get('/api/billing/me', authenticate, async (req, res) => {
  try {
    ...
    const month   = new Date().toISOString().slice(0, 7);
    const sameMonth       = usage.kieResetMonth === month;
    const kieUsedThisMonth = sameMonth ? (usage.kieCount || 0) : 0;
    const kieTopupLeft    = sameMonth ? (usage.kieTopupRemaining || 0) : 0;
...
app.post('/api/kie', authenticate, async (req, res) => {
  ...
  if (mode === 'web_search' && !planCfg.kieWebSearch) {
    return res.status(403).json({
      error: 'mode_locked',
