...
async function sendWelcomeEmail(email, name) {
  ...
  try {
    const res = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender:     { email: 'support@goresumaker.online', name: 'Resumaker' },
        to:         [{ email, name }],
        templateId: 1,
        params:     { name },
      }),
...
app.get('/api/check-username', async (req, res) => {
  ...
  if (!raw) return res.json({ available: false, error: 'No username provided' });
...
app.post('/api/register-user', authenticate, async (req, res) => {
  ...
  try {
    // If a username was provided, atomically reserve it
    if (cleanUsername && /^[a-z0-9_]{3,20}$/.test(cleanUsername)) {
      ...
      // Reserve username doc
      await usernameRef.set({ uid, createdAt: admin.firestore.FieldValue.serverTimestamp() });
    }
    ...
    console.log(`👤 User doc created for ${email}${cleanUsername ? ' (@' + cleanUsername + ')' : ''}`);
...
// Atomically reserves a new username and releases the old one, server-side.
// Admin SDK bypasses Firestore rules — safe because we enforce ownership here.
app.post('/api/claim-username', authenticate, async (req, res) => {
  ...
  try {
    const usernameRef = db.collection('usernames').doc(clean);
    const snap = await usernameRef.get();
    ...
    // Atomic batch: reserve new + release old + update user doc
    const batch = db.batch();
    batch.set(usernameRef, { uid, createdAt: admin.firestore.FieldValue.serverTimestamp() });
...
Concise, friendly, conversational. Max 3-4 sentences unless a step-by-step is genuinely needed.`;
  ...
  try {
    ...
    const data  = await groqRes.json();
    const reply = data.choices?.[0]?.message?.content || "Sorry, I couldn't get a response right now. Try emailing support@goresumaker.online";
    res.json({ reply });
  } catch (err) {
