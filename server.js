...
Be decisive and exact. When one option is clearly stronger, name it and move on — don't lay out three choices and ask the user to pick for themselves. One precise insight beats three shallow ones. Every claim should trace back to something this specific person told you; generic advice is the one failure mode to avoid at this tier.`,
  ...
  pro: {
    tokenBonus: 250,
    system: `
INTELLIGENCE TIER — KIE PRO (Deep Career Intelligence):
...
RESUME PDF TRIGGER:
If the user has a resume loaded and asks you to apply changes AND resend/send the PDF, end your reply with [SEND_PDF] on its own line. Do NOT add [SEND_PDF] unless the user has explicitly asked for the PDF to be resent.
...
FOLLOW-UP CHIPS: At the end of substantive responses (not short replies, not responses ending in a question), include 2–3 short follow-up suggestions the user might genuinely want to do next. Format each at the very end of your reply: [FU]suggestion text[/FU]. Make them specific to what was just discussed — real next actions. Example after startup discussion: [FU]Help me write an investor pitch[/FU] [FU]How do I validate my idea fast?[/FU] [FU]Build my personal brand as a founder[/FU]. Example after resume edit: [FU]Improve my work experience bullets[/FU] [FU]Check my ATS score[/FU]. Never use [FU] chips on greetings, short replies, or when your response ends with a question.`,
    label:       'Deep Think',
    max_tokens:  1700,
    temperature: 0.55,
...
FOLLOW-UP CHIPS: At the end of substantive responses (not short replies, not responses ending in a question), include 2–3 short follow-up suggestions the user might genuinely want to do next. Format each at the very end: [FU]suggestion text[/FU]. Make them specific to the topic just discussed. Never use on greetings, short replies, or when ending with a question.`,
    label:       'Web Search',
    max_tokens:  1000,
    temperature: 0.65,
    system: `You are KIE in Market Intel mode — a career mentor built by Kievora with deep knowledge of job markets, industries, salary data, hiring trends, and in-demand skills. You are the person in the room who actually knows what's happening out there.
...
HONESTY RULE — CRITICAL: You do NOT have live internet access. Never claim you just searched the web. Frame your knowledge honestly: "industry patterns consistently show…", "hiring data from platforms like LinkedIn and Glassdoor has historically indicated…". When a space moves fast, say so.
...
7. HONEST ABOUT LIMITS — Fast-moving spaces get flagged to verify on Glassdoor/LinkedIn Salary.
...
app.post('/api/kie', authenticate, async (req, res) => {
  ...
  // Quick Answer mode is intentionally short on every engine — don't let a
  // higher tier inflate it back out. Every other mode gets the tier's full
  ...
  // Mode prompt sets WHO KIE is and the output format; the tier layer sets HOW
  // hard KIE thinks before answering, calibrated to what the underlying engine
  ...
  const hasVision = m.provider === 'anthropic'; // Anthropic supports vision; Groq does not
  ...
  const formattedMessages = messages.map(msg => {
      ...
      if (hasVision) {
        ...
        return {
          ...
          content: [
            {
              type:   'image',
              source: { type: 'base64', media_type: msg.imageType, data: msg.imageBase64 },
            },
            {
              type: 'text',
              text: msg.content || 'What do you see in this image? Give career coaching advice based on it.',
            },
          ],
        };
      } else {
        // Groq — no vision, strip image and add text note
        return {
          role:    msg.role,
    ...
    // Standard text message — strip any image fields
    return { role: msg.role, content: msg.content || '' };
  });

  console.log(`POST /api/kie — model: ${effectiveModel}(${m.model}) mode: ${effectiveMode} messages: ${messages.length} hasImage: ${messages.some(msg => msg.imageBase64)}`);

  try {
    const reply = await callKieAI(effectiveModel, systemContent, formattedMessages, effectiveCfg);
    res.json({ reply, mode: effectiveMode, model: effectiveModel });

    // ── Admin logging (fire-and-forget, never blocks response) ───────────────
    ...
    db.collection('analyticsEvents').add({
      ...
      model: effectiveModel, mode: effectiveMode,
    ...
    if (effectiveModel !== 'spark') {
      ...
      try {
        // For fallback, strip images since Groq doesn't support vision
        const textOnlyMessages = formattedMessages.map(msg => ({
          role:    msg.role,
          content: typeof msg.content === 'string' ? msg.content : (msg.content?.find?.(b => b.type === 'text')?.text || ''),
        }));
        const reply = await callKieAI('spark', systemContent, textOnlyMessages, { ...cfg });
