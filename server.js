...
app.post('/api/billing/webhook', express.raw({ type: '*/*' }), async (req, res) => {
  try {
    ...
    if (event.event === 'charge.success') {
      ...
      if (type === 'topup') {
        // Top-up adds to the CURRENT cycle — it never changes the subscription
        // anchor, so we compute the cycle start from whatever anchor the user
        // already has (planUpdatedAt for paid, createdAt for free/edge cases).
        ...
        // Plan upgrade/change resets the billing anchor to right now — every
        // subscribe or upgrade starts a brand new cycle. We deliberately don't
        // write kieCycleStart here; the next /api/kie call computes it fresh
        // from the new planUpdatedAt, avoiding any clock-drift mismatch between
        // this serverTimestamp() and a manually-stamped cycle-start string.
