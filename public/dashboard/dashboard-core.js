  ...
  measurementId: "G-X6VG3MVQ96"
        ...
        return `<div class="rr-slide" onclick="${openAction}">
          <div class="rr-card">
            <div class="rr-thumb">
              ...
              <div class="rr-doc-badge">${badgeInner}</div>
            </div>
            <div class="rr-info">
            ...
            <div class="rr-actions">
              <button class="rr-btn rr-btn-edit" onclick="event.stopPropagation();${editAction}" title="Edit">
              ...
              <button class="rr-btn rr-btn-del" onclick="event.stopPropagation();${delAction}" title="Delete">
    ...
    function secCert(d, c, mode) {
      ...
      return `<div style="margin-bottom:14px"><div style="font-size:12.5px;font-weight:900;color:${t};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px">Certifications</div>${d.certifications.map(x=>`<div style="margin-bottom:6px"><strong style="font-size:15px;color:${tx}">${esc(x.name)}</strong>${x.issuer?`<div style="font-size:14px;color:${sub}">${esc(x.issuer)}${x.date?' · '+esc(x.date):''}</div>`:''}</div>`).join('')}</div>`;
    }
    function secProj(d, c, mode) {
      ...
      const tx = dark ? 'rgba(255,255,255,.92)' : '#111827';
      const sub = dark ? 'rgba(255,255,255,.62)' : '#6b7280';
      return `<div style="margin-bottom:14px"><div style="font-size:12.5px;font-weight:900;color:${t};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px">Projects</div>${d.projects.map(x=>`<div style="margin-bottom:8px"><strong style="font-size:15px;color:${tx}">${esc(x.name)}</strong>${x.url?`<div style="font-size:13px;color:${dark?'rgba(255,255,255,.75)':c}">${esc(x.url)}</div>`:''}${x.description?`<p style="font-size:14px;color:${sub};margin-top:2px;line-height:1.5">${esc(x.description)}</p>`:''}</div>`).join('')}</div>`;
    }
    function secLang(d, c, mode) {
      ...
      return `<div style="margin-bottom:14px"><div style="font-size:12.5px;font-weight:900;color:${t};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px">Languages</div><div style="display:flex;flex-wrap:wrap;gap:7px">${d.languages.map(x=>`<span style="font-size:14px;color:${tx}">${esc(x.language)}${x.proficiency?` <span style="color:${sub}">(${esc(x.proficiency)})</span>`:''}</span>`).join('')}</div></div>`;
    }

    function tplClassic(d,c,fc){return`<div class="${fc}" style="font-weight:500;padding:32px;font-size:17.5px;line-height:1.65;background:#fff;min-height:700px">
      <div style="border-bottom:3px solid ${c};padding-bottom:12px;margin-bottom:16px;display:flex;align-items:flex-start;gap:16px">
        ...
        <div style="flex:1"><h1 style="font-size:35px;font-weight:800;color:${c};letter-spacing:-.3px">${esc(d.fullName||'Your Name')}</h1>
        <p style="font-size:19px;color:#555;margin-top:2px;font-weight:600">${esc(d.jobTitle||'')}</p>
        <p style="color:#888;font-size:14.5px;margin-top:5px">${[d.email,d.phone,d.location].filter(Boolean).map(esc).join('  ·  ')}</p></div>
      </div>
      ${d.summary?`<div style="margin-bottom:14px"><div style="font-size:13px;font-weight:800;color:${c};text-transform:uppercase;letter-spacing:1px;margin-bottom:5px">Professional Summary</div><p style="color:#444;font-size:16px">${esc(d.summary)}</p></div>`:''}
      ${(d.workExperience||[]).length?`<div style="margin-bottom:14px"><div style="font-size:13px;font-weight:800;color:${c};text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Work Experience</div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:flex-start"><strong style="font-size:16px">${esc(w.position)}</strong><span style="color:#999;font-size:13px;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Present')}</span></div><div style="color:#6b7280;font-size:14.5px;margin-top:1px">${esc(w.company)}</div>${w.description?`<p style="color:#777;font-size:14.5px;margin-top:3px;line-height:1.5">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
      ${(d.education||[]).length?`<div style="margin-bottom:14px"><div style="font-size:13px;font-weight:800;color:${c};text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Education</div>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><div style="display:flex;justify-content:space-between"><strong style="font-size:16px">${esc(e.degree)} in ${esc(e.field)}</strong><span style="color:#999;font-size:13px">${esc(e.graduationDate)}</span></div><div style="color:#6b7280;font-size:14.5px">${esc(e.school)}</div></div>`).join('')}</div>`:''}
      ${(d.skills||[]).length?`<div style="margin-bottom:14px"><div style="font-size:13px;font-weight:800;color:${c};text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Skills</div><div style="display:flex;flex-wrap:wrap;gap:5px">${(d.skills||[]).map(s=>`<span style="background:${c}18;color:${c};padding:2px 10px;border-radius:99px;font-size:14.5px;font-weight:700">${esc(s)}</span>`).join('')}</div></div>`:''}
      ${secProj(d,c)}${secCert(d,c)}${secLang(d,c)}
    </div>`;}

    function tplModern(d,c,fc){return`<div class="${fc}" style="font-weight:500;font-size:17.5px;display:grid;grid-template-columns:190px 1fr;background:#fff;min-height:700px">
      <div style="background:${c};color:#fff;padding:24px 16px">
        ${d.photo?`<img src="${d.photo}" alt="" style="width:60px;height:60px;border-radius:50%;object-fit:cover;margin-bottom:12px;border:2px solid rgba(255,255,255,.5);display:block">`:` <div style="width:60px;height:60px;border-radius:50%;background:rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;font-size:35px;font-weight:900;margin-bottom:12px;border:2px solid rgba(255,255,255,.3)">${(d.fullName||'?')[0].toUpperCase()}</div>`}
        <h1 style="font-size:22px;font-weight:800;line-height:1.3;margin-bottom:3px">${esc(d.fullName||'Your Name')}</h1>
        <p style="opacity:.8;font-size:16px;font-weight:600">${esc(d.jobTitle||'')}</p>
        <div style="margin-top:16px;font-size:14.5px;opacity:.85;line-height:2.0">${d.email?`<p>✉ ${esc(d.email)}</p>`:''} ${d.phone?`<p>☎ ${esc(d.phone)}</p>`:''} ${d.location?`<p>⌖ ${esc(d.location)}</p>`:''}</div>
        ${(d.skills||[]).length?`<div style="margin-top:20px"><p style="font-size:11.5px;font-weight:800;text-transform:uppercase;letter-spacing:1px;opacity:.6;margin-bottom:8px">Skills</p>${(d.skills||[]).map(s=>`<div style="background:rgba(255,255,255,.15);border-radius:5px;padding:3px 9px;font-size:14.5px;margin-bottom:4px;border-left:3px solid rgba(255,255,255,.4)">${esc(s)}</div>`).join('')}</div>`:''}
      </div>
      <div style="padding:24px 22px;line-height:1.6">
        ${d.summary?`<div style="margin-bottom:16px"><p style="font-size:13px;font-weight:800;color:${c};text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">About Me</p><p style="color:#444;font-size:16px">${esc(d.summary)}</p></div>`:''}
        ${(d.workExperience||[]).length?`<div style="margin-bottom:14px"><p style="font-size:13px;font-weight:800;color:${c};text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Experience</p>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16px">${esc(w.position)}</strong><div style="color:#6b7280;font-size:14.5px">${esc(w.company)}</div></div><span style="color:#999;font-size:13px;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Present')}</span></div>${w.description?`<p style="color:#666;font-size:14.5px;margin-top:3px">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
        ${(d.education||[]).length?`<div style="margin-bottom:14px"><p style="font-size:13px;font-weight:800;color:${c};text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Education</p>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><strong style="font-size:16px">${esc(e.degree)} in ${esc(e.field)}</strong><div style="color:#6b7280;font-size:14.5px">${esc(e.school)} · ${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
        ${secProj(d,c)}${secCert(d,c)}${secLang(d,c)}
      </div>
    </div>`;}

    function tplBold(d,c,fc){return`<div class="${fc}" style="font-weight:500;font-size:17.5px;background:#fff;min-height:700px">
      <div style="background:${c};padding:28px 32px;color:#fff">
        <h1 style="font-size:43.5px;font-weight:900;text-transform:uppercase;letter-spacing:-1px">${esc(d.fullName||'YOUR NAME')}</h1>
        <p style="font-size:20.5px;opacity:.85;margin-top:3px;font-weight:700;letter-spacing:.5px">${esc(d.jobTitle||'')}</p>
        <div style="display:flex;flex-wrap:wrap;gap:16px;margin-top:10px;font-size:14.5px;opacity:.8">${d.email?`<span>✉ ${esc(d.email)}</span>`:''} ${d.phone?`<span>☎ ${esc(d.phone)}</span>`:''} ${d.location?`<span>⌖ ${esc(d.location)}</span>`:''}</div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 180px">
        <div style="padding:24px 28px;border-right:2px solid #f8f7ff">
          ${d.summary?`<div style="margin-bottom:16px"><div style="font-size:13px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid ${c};padding-bottom:4px;margin-bottom:8px">Profile</div><p style="color:#444;font-size:16px">${esc(d.summary)}</p></div>`:''}
          ${(d.workExperience||[]).length?`<div><div style="font-size:13px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid ${c};padding-bottom:4px;margin-bottom:10px">Experience</div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between"><strong style="font-size:16px">${esc(w.position)}</strong><span style="color:#999;font-size:13px">${esc(w.startDate)} – ${esc(w.endDate||'Now')}</span></div><div style="color:#6b7280;font-size:14.5px">${esc(w.company)}</div>${w.description?`<p style="color:#555;font-size:14.5px;margin-top:3px">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
        </div>
        <div style="padding:24px 16px;background:#faf9ff">
          ${(d.skills||[]).length?`<div style="margin-bottom:16px"><div style="font-size:11.5px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid ${c};padding-bottom:3px;margin-bottom:8px">Skills</div>${(d.skills||[]).map(s=>`<div style="margin-bottom:5px;font-size:14.5px;color:#374151;display:flex;align-items:center;gap:5px"><span style="width:5px;height:5px;border-radius:50%;background:${c};flex-shrink:0"></span>${esc(s)}</div>`).join('')}</div>`:''}
          ${(d.education||[]).length?`<div style="margin-bottom:14px"><div style="font-size:11.5px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid ${c};padding-bottom:3px;margin-bottom:8px">Education</div>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><strong style="font-size:14.5px">${esc(e.degree)}</strong><div style="color:#7c3aed;font-size:13px">${esc(e.field)}</div><div style="color:#6b7280;font-size:13px">${esc(e.school)}</div><div style="color:#999;font-size:13px">${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
          ${secProj(d,c)}${secCert(d,c)}${secLang(d,c)}
        </div>
      </div>
    </div>`;}

    function tplMinimal(d,c,fc){return`<div class="${fc}" style="font-weight:500;padding:40px 44px;font-size:17.5px;background:#fff;min-height:700px;line-height:1.7">
      <div style="margin-bottom:24px">
        <h1 style="font-size:40.5px;font-weight:600;color:#111;letter-spacing:-.5px">${esc(d.fullName||'Your Name')}</h1>
        <p style="font-size:19px;color:${c};font-weight:700;margin-top:2px">${esc(d.jobTitle||'')}</p>
        <p style="color:#aaa;font-size:14.5px;margin-top:6px;letter-spacing:.3px">${[d.email,d.phone,d.location].filter(Boolean).map(esc).join('   ·   ')}</p>
      </div>
      ${d.summary?`<div style="margin-bottom:20px;padding-top:16px;border-top:1px solid #f1f5f9"><p style="font-size:13px;color:#aaa;text-transform:uppercase;letter-spacing:2px;font-weight:800;margin-bottom:6px">About</p><p style="color:#555;font-size:16px">${esc(d.summary)}</p></div>`:''}
      ${(d.workExperience||[]).length?`<div style="margin-bottom:20px;padding-top:16px;border-top:1px solid #f1f5f9"><p style="font-size:13px;color:#aaa;text-transform:uppercase;letter-spacing:2px;font-weight:800;margin-bottom:10px">Experience</p>${(d.workExperience||[]).map(w=>`<div style="display:flex;gap:20px;margin-bottom:10px"><div style="color:#ccc;font-size:13px;white-space:nowrap;padding-top:2px;min-width:60px">${esc(w.startDate)}<br>– ${esc(w.endDate||'Now')}</div><div><strong style="font-size:16px;color:#111">${esc(w.position)}</strong><div style="color:#666;font-size:14.5px">${esc(w.company)}</div>${w.description?`<p style="color:#777;font-size:14.5px;margin-top:2px">${esc(w.description)}</p>`:''}</div></div>`).join('')}</div>`:''}
      ${(d.education||[]).length?`<div style="margin-bottom:20px;padding-top:16px;border-top:1px solid #f1f5f9"><p style="font-size:13px;color:#aaa;text-transform:uppercase;letter-spacing:2px;font-weight:800;margin-bottom:10px">Education</p>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><strong style="font-size:16px;color:#111">${esc(e.degree)} in ${esc(e.field)}</strong><div style="color:#666;font-size:14.5px">${esc(e.school)} · ${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
      ${(d.skills||[]).length?`<div style="margin-bottom:20px;padding-top:16px;border-top:1px solid #f1f5f9"><p style="font-size:13px;color:#aaa;text-transform:uppercase;letter-spacing:2px;font-weight:800;margin-bottom:8px">Skills</p><div style="display:flex;flex-wrap:wrap;gap:6px">${(d.skills||[]).map(s=>`<span style="border:1.5px solid #e2e8f0;color:#374151;padding:3px 12px;border-radius:5px;font-size:14.5px">${esc(s)}</span>`).join('')}</div></div>`:''}
    ...
    function tplVivid(d,c,fc){return`<div class="${fc}" style="font-weight:500;font-size:17.5px;background:#fff;min-height:700px">
      <div style="background:linear-gradient(135deg,${c},${c}bb);padding:30px 32px;color:#fff">
        <h1 style="font-size:40.5px;font-weight:900;letter-spacing:-.5px">${esc(d.fullName||'Your Name')}</h1>
        <p style="font-size:20.5px;opacity:.9;margin-top:3px;font-weight:700">${esc(d.jobTitle||'')}</p>
        <div style="display:flex;flex-wrap:wrap;gap:14px;margin-top:10px;font-size:14.5px;opacity:.85">${d.email?`<span>✉ ${esc(d.email)}</span>`:''} ${d.phone?`<span>☎ ${esc(d.phone)}</span>`:''} ${d.location?`<span>⌖ ${esc(d.location)}</span>`:''}</div>
      </div>
      <div style="padding:24px 32px">
        ${d.summary?`<div style="margin-bottom:16px;padding:14px 16px;background:${c}0d;border-left:4px solid ${c};border-radius:0 8px 8px 0"><p style="color:#444;font-size:16px;line-height:1.6">${esc(d.summary)}</p></div>`:''}
        ${(d.workExperience||[]).length?`<div style="margin-bottom:16px"><h3 style="font-size:19px;font-weight:800;color:${c};margin-bottom:10px">Work Experience</h3>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:10px;padding-left:12px;border-left:3px solid ${c}55"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16px">${esc(w.position)}</strong><div style="color:#6b7280;font-size:14.5px">${esc(w.company)}</div></div><span style="color:#aaa;font-size:13px;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Present')}</span></div>${w.description?`<p style="color:#555;font-size:14.5px;margin-top:3px">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
        ${(d.education||[]).length?`<div style="margin-bottom:16px"><h3 style="font-size:19px;font-weight:800;color:${c};margin-bottom:10px">Education</h3>${(d.education||[]).map(e=>`<div style="margin-bottom:8px;padding-left:12px;border-left:3px solid ${c}55"><strong style="font-size:16px">${esc(e.degree)} in ${esc(e.field)}</strong><div style="color:#6b7280;font-size:14.5px">${esc(e.school)} · ${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
        ${(d.skills||[]).length?`<div style="margin-bottom:16px"><h3 style="font-size:19px;font-weight:800;color:${c};margin-bottom:8px">Skills</h3><div style="display:flex;flex-wrap:wrap;gap:6px">${(d.skills||[]).map(s=>`<span style="background:${c};color:#fff;padding:3px 12px;border-radius:99px;font-size:14.5px;font-weight:700">${esc(s)}</span>`).join('')}</div></div>`:''}
        ${secProj(d,c)}${secCert(d,c)}${secLang(d,c)}
      </div>
    </div>`;}
    ...
    function tplElegant(d,c,fc){return`<div class="${fc}" style="font-weight:500;padding:36px 40px;font-size:17.5px;background:#fff;min-height:700px;font-family:Georgia,serif;line-height:1.7">
      <div style="text-align:center;padding-bottom:18px;border-bottom:1px solid #d6d3d1;margin-bottom:20px">
        ...
        <h1 style="font-size:37.5px;font-weight:800;color:#1c1917;letter-spacing:.5px;margin-bottom:4px">${esc(d.fullName||'Your Name')}</h1>
        <p style="font-size:17.5px;color:${c};font-weight:700;letter-spacing:2px;text-transform:uppercase;margin-bottom:6px">${esc(d.jobTitle||'')}</p>
        <p style="color:#a8a29e;font-size:14px;letter-spacing:.4px">${[d.email,d.phone,d.location].filter(Boolean).map(esc).join('  ·  ')}</p>
      </div>
      ${d.summary?`<div style="margin-bottom:18px;text-align:center"><p style="color:#57534e;font-size:16px;max-width:420px;margin:0 auto;font-style:italic;line-height:1.8">${esc(d.summary)}</p></div>`:''}
      ${(d.workExperience||[]).length?`<div style="margin-bottom:18px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:12px"><div style="height:1px;flex:1;background:#e7e5e4"></div><span style="font-size:12.5px;font-weight:800;color:${c};letter-spacing:2px;text-transform:uppercase;font-family:'Inter',sans-serif">Experience</span><div style="height:1px;flex:1;background:#e7e5e4"></div></div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;align-items:baseline"><div><strong style="font-size:16.5px;color:#1c1917">${esc(w.position)}</strong><span style="color:#a8a29e;margin:0 6px">—</span><span style="font-size:15px;color:#78716c">${esc(w.company)}</span></div><span style="color:#a8a29e;font-size:13px;font-family:'Inter',sans-serif">${esc(w.startDate)} – ${esc(w.endDate||'Present')}</span></div>${w.description?`<p style="color:#6b7280;font-size:14.5px;margin-top:3px;font-style:italic">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
      ${(d.education||[]).length?`<div style="margin-bottom:18px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:12px"><div style="height:1px;flex:1;background:#e7e5e4"></div><span style="font-size:12.5px;font-weight:800;color:${c};letter-spacing:2px;text-transform:uppercase;font-family:'Inter',sans-serif">Education</span><div style="height:1px;flex:1;background:#e7e5e4"></div></div>${(d.education||[]).map(e=>`<div style="display:flex;justify-content:space-between;margin-bottom:8px"><div><strong style="font-size:16px;color:#1c1917">${esc(e.degree)} in ${esc(e.field)}</strong><div style="color:#78716c;font-size:14.5px">${esc(e.school)}</div></div><span style="color:#a8a29e;font-size:13px;font-family:'Inter',sans-serif">${esc(e.graduationDate)}</span></div>`).join('')}</div>`:''}
      ${(d.skills||[]).length?`<div style="margin-bottom:18px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px"><div style="height:1px;flex:1;background:#e7e5e4"></div><span style="font-size:12.5px;font-weight:800;color:${c};letter-spacing:2px;text-transform:uppercase;font-family:'Inter',sans-serif">Skills</span><div style="height:1px;flex:1;background:#e7e5e4"></div></div><div style="display:flex;flex-wrap:wrap;justify-content:center;gap:5px">${(d.skills||[]).map(s=>`<span style="font-size:14px;color:#57534e;border:1px solid #d6d3d1;padding:2px 11px;border-radius:2px;font-family:'Inter',sans-serif">${esc(s)}</span>`).join('')}</div></div>`:''}
      ${secProj(d,c)}${secCert(d,c)}${secLang(d,c)}
    </div>`;}

    // SLATE — dark left panel with timeline experience, for senior/tech
    function tplSlate(d,c,fc){return`<div class="${fc}" style="font-weight:500;font-size:17.5px;display:grid;grid-template-columns:200px 1fr;background:#fff;min-height:700px">
      <div style="background:#0f172a;color:#e2e8f0;padding:28px 18px;display:flex;flex-direction:column;gap:0">
        <div style="margin-bottom:20px">
          ${d.photo?`<img src="${d.photo}" alt="" style="width:52px;height:52px;border-radius:12px;object-fit:cover;margin-bottom:14px;border:2px solid ${c};display:block">`:` <div style="width:52px;height:52px;border-radius:12px;background:${c};display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:900;color:#fff;margin-bottom:14px">${(d.fullName||'?')[0].toUpperCase()}</div>`}
          <h1 style="font-size:20.5px;font-weight:800;line-height:1.35;margin-bottom:4px;color:#f1f5f9">${esc(d.fullName||'Your Name')}</h1>
          <p style="font-size:14.5px;color:${c};font-weight:800;letter-spacing:.5px">${esc(d.jobTitle||'')}</p>
        </div>
        <div style="margin-bottom:18px;font-size:14px;line-height:2;color:#94a3b8">${d.email?`<div>✉ ${esc(d.email)}</div>`:''} ${d.phone?`<div>☎ ${esc(d.phone)}</div>`:''} ${d.location?`<div>⌖ ${esc(d.location)}</div>`:''}</div>
        ${(d.skills||[]).length?`<div><p style="font-size:11.5px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:#475569;margin-bottom:9px">Skills</p><div style="display:flex;flex-direction:column;gap:4px">${(d.skills||[]).map(s=>`<div style="font-size:14.5px;color:#cbd5e1;padding:4px 10px;background:rgba(255,255,255,.06);border-radius:5px;border-left:3px solid ${c}">${esc(s)}</div>`).join('')}</div></div>`:''}
        ${(d.education||[]).length?`<div style="margin-top:18px"><p style="font-size:11.5px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:#475569;margin-bottom:9px">Education</p>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><p style="font-size:14.5px;font-weight:800;color:#e2e8f0">${esc(e.degree)}</p><p style="font-size:13px;color:#94a3b8">${esc(e.field)}</p><p style="font-size:13px;color:#64748b">${esc(e.school)} · ${esc(e.graduationDate)}</p></div>`).join('')}</div>`:''}
      ...
      <div style="padding:28px 24px;background:#fff">
        ${d.summary?`<div style="margin-bottom:18px;padding:12px 14px;background:#f8fafc;border-left:4px solid ${c};border-radius:0 6px 6px 0"><p style="font-size:16px;color:#334155;line-height:1.7">${esc(d.summary)}</p></div>`:''}
        ${(d.workExperience||[]).length?`<div style="margin-bottom:16px"><p style="font-size:11.5px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:#94a3b8;margin-bottom:14px">Experience</p><div style="position:relative;padding-left:20px;border-left:2px solid #e2e8f0">${(d.workExperience||[]).map(w=>`<div style="position:relative;margin-bottom:14px"><div style="position:absolute;left:-25px;top:3px;width:8px;height:8px;border-radius:50%;background:${c};border:2px solid #fff;box-shadow:0 0 0 2px ${c}"></div><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16.5px;color:#0f172a">${esc(w.position)}</strong><div style="font-size:14.5px;color:#64748b">${esc(w.company)}</div></div><span style="font-size:13px;color:#94a3b8;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Now')}</span></div>${w.description?`<p style="font-size:14.5px;color:#475569;margin-top:3px;line-height:1.6">${esc(w.description)}</p>`:''}</div>`).join('')}</div></div>`:''}
        ${secProj(d,c)}
      </div>
    </div>`;}

    // CORAL — warm orange accent, card-based sections, energetic layout
    function tplCoral(d,c,fc){return`<div class="${fc}" style="font-weight:500;font-size:17.5px;background:#fff;min-height:700px">
      <div style="background:linear-gradient(120deg,${c} 0%,#ea580c 60%,#fb923c 100%);padding:30px 32px;color:#fff;position:relative;overflow:hidden">
        ...
        <div style="position:relative">
          <h1 style="font-size:37.5px;font-weight:900;letter-spacing:-.5px;margin-bottom:4px">${esc(d.fullName||'Your Name')}</h1>
          <p style="font-size:17.5px;opacity:.9;font-weight:700;letter-spacing:.5px;margin-bottom:10px">${esc(d.jobTitle||'')}</p>
          <div style="display:flex;flex-wrap:wrap;gap:12px;font-size:14px;opacity:.85">${d.email?`<span style="background:rgba(255,255,255,.15);padding:2px 10px;border-radius:99px">✉ ${esc(d.email)}</span>`:''} ${d.phone?`<span style="background:rgba(255,255,255,.15);padding:2px 10px;border-radius:99px">☎ ${esc(d.phone)}</span>`:''} ${d.location?`<span style="background:rgba(255,255,255,.15);padding:2px 10px;border-radius:99px">⌖ ${esc(d.location)}</span>`:''}</div>
        </div>
      </div>
      <div style="padding:22px 28px">
        ${d.summary?`<div style="margin-bottom:16px;padding:12px 16px;background:#fff7ed;border-radius:10px;border:1px solid #fed7aa"><p style="font-size:16px;color:#431407;line-height:1.75">${esc(d.summary)}</p></div>`:''}
        ${(d.workExperience||[]).length?`<div style="margin-bottom:16px"><div style="font-size:13px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px;display:flex;align-items:center;gap:8px">Experience<div style="flex:1;height:2px;background:#fed7aa;border-radius:99px"></div></div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:10px;padding:10px 14px;border-radius:8px;border:1px solid #f3f4f6;background:#fafafa"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16px;color:#1c1917">${esc(w.position)}</strong><div style="font-size:14px;color:${c};font-weight:700;margin-top:1px">${esc(w.company)}</div></div><span style="font-size:13px;color:#a8a29e;white-space:nowrap">${esc(w.startDate)} – ${esc(w.endDate||'Now')}</span></div>${w.description?`<p style="font-size:14.5px;color:#57534e;margin-top:5px;line-height:1.6">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          ${(d.education||[]).length?`<div><div style="font-size:13px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px">Education</div>${(d.education||[]).map(e=>`<div style="margin-bottom:7px;padding:8px 12px;background:#fff7ed;border-radius:8px"><strong style="font-size:14.5px;color:#1c1917">${esc(e.degree)} in ${esc(e.field)}</strong><div style="font-size:13px;color:#78716c">${esc(e.school)} · ${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
          ${(d.skills||[]).length?`<div><div style="font-size:13px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:1.5px;margin-bottom:8px">Skills</div><div style="display:flex;flex-wrap:wrap;gap:5px">${(d.skills||[]).map(s=>`<span style="background:${c};color:#fff;padding:3px 10px;border-radius:99px;font-size:14px;font-weight:700">${esc(s)}</span>`).join('')}</div></div>`:''}
        </div>
    ...
    function tplSplit(d,c,fc){return`<div class="${fc}" style="font-weight:500;font-size:17.5px;display:grid;grid-template-columns:170px 1fr;background:#fff;min-height:700px">
      <div style="background:${c};padding:28px 16px;display:flex;flex-direction:column;align-items:center;text-align:center">
        ${d.photo?`<img src="${d.photo}" alt="" style="width:64px;height:64px;border-radius:50%;object-fit:cover;margin-bottom:14px;border:2.5px solid rgba(255,255,255,.6);display:block">`:` <div style="width:64px;height:64px;border-radius:16px;background:rgba(255,255,255,.18);border:2.5px solid rgba(255,255,255,.35);display:flex;align-items:center;justify-content:center;font-size:40.5px;font-weight:900;color:#fff;margin-bottom:14px">${(d.fullName||'?')[0].toUpperCase()}</div>`}
        <h1 style="font-size:19px;font-weight:800;color:#fff;line-height:1.4;margin-bottom:4px">${esc(d.fullName||'Your Name')}</h1>
        <p style="font-size:14px;color:rgba(255,255,255,.75);font-weight:700;margin-bottom:16px">${esc(d.jobTitle||'')}</p>
        <div style="width:100%;height:1px;background:rgba(255,255,255,.2);margin-bottom:14px"></div>
        <div style="width:100%;text-align:left;font-size:14px;color:rgba(255,255,255,.8);line-height:2.0">${d.email?`<div style="word-break:break-all">${esc(d.email)}</div>`:''} ${d.phone?`<div>${esc(d.phone)}</div>`:''} ${d.location?`<div>${esc(d.location)}</div>`:''}</div>
        ${(d.skills||[]).length?`<div style="margin-top:16px;width:100%"><p style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:rgba(255,255,255,.5);margin-bottom:7px;text-align:left">Skills</p>${(d.skills||[]).map(s=>`<div style="background:rgba(255,255,255,.12);border-radius:5px;padding:3px 8px;font-size:14px;color:rgba(255,255,255,.9);margin-bottom:4px;text-align:left">${esc(s)}</div>`).join('')}</div>`:''}
      </div>
      <div style="padding:28px 24px">
        ${d.summary?`<div style="margin-bottom:16px"><div style="font-size:11.5px;font-weight:900;color:#94a3b8;text-transform:uppercase;letter-spacing:2px;margin-bottom:6px">About</div><p style="color:#334155;font-size:16px;line-height:1.75">${esc(d.summary)}</p></div>`:''}
        ${(d.workExperience||[]).length?`<div style="margin-bottom:14px"><div style="font-size:11.5px;font-weight:900;color:#94a3b8;text-transform:uppercase;letter-spacing:2px;margin-bottom:10px">Work Experience</div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid #f1f5f9"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16.5px;color:#0f172a">${esc(w.position)}</strong><div style="font-size:14.5px;color:${c};font-weight:700">${esc(w.company)}</div></div><span style="font-size:13px;color:#94a3b8;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Present')}</span></div>${w.description?`<p style="font-size:14.5px;color:#475569;margin-top:4px;line-height:1.6">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
        ${(d.education||[]).length?`<div style="margin-bottom:14px"><div style="font-size:11.5px;font-weight:900;color:#94a3b8;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px">Education</div>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><strong style="font-size:16px;color:#0f172a">${esc(e.degree)} in ${esc(e.field)}</strong><div style="font-size:14.5px;color:${c};font-weight:700">${esc(e.school)}</div><div style="font-size:13px;color:#94a3b8">${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
        ${secProj(d,c)}${secCert(d,c)}${secLang(d,c)}
      </div>
    </div>`;}

    // INK — high-contrast editorial; black header strip, stark typography
    function tplInk(d,c,fc){return`<div class="${fc}" style="font-weight:500;font-size:17.5px;background:#fff;min-height:700px">
      <div style="background:#111827;padding:28px 32px">
        <div style="display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;gap:12px">
          <div>
            <h1 style="font-size:43.5px;font-weight:900;color:#fff;letter-spacing:-1.5px;line-height:1;margin-bottom:5px">${esc(d.fullName||'Your Name')}</h1>
            <p style="font-size:17.5px;font-weight:700;color:${c};letter-spacing:1px;text-transform:uppercase">${esc(d.jobTitle||'')}</p>
          </div>
          <div style="text-align:right;font-size:14px;color:#9ca3af;line-height:2.0">${d.email?`<div>${esc(d.email)}</div>`:''} ${d.phone?`<div>${esc(d.phone)}</div>`:''} ${d.location?`<div>${esc(d.location)}</div>`:''}</div>
        </div>
      </div>
      <div style="height:4px;background:linear-gradient(90deg,${c},${c}66,transparent)"></div>
      <div style="padding:24px 32px">
        ${d.summary?`<div style="margin-bottom:18px;padding-bottom:16px;border-bottom:1.5px solid #f1f5f9"><p style="font-size:16.5px;color:#374151;line-height:1.8">${esc(d.summary)}</p></div>`:''}
        ${(d.workExperience||[]).length?`<div style="margin-bottom:18px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:12px"><span style="font-size:12.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111827">Experience</span><div style="flex:1;height:2px;background:#111827"></div></div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:12px;display:grid;grid-template-columns:1fr auto;gap:8px"><div><div style="display:flex;align-items:center;gap:8px;margin-bottom:2px"><strong style="font-size:16.5px;color:#111827">${esc(w.position)}</strong><span style="font-size:13px;color:#fff;background:${c};padding:1px 8px;border-radius:3px;font-weight:800">${esc(w.company)}</span></div>${w.description?`<p style="font-size:14.5px;color:#6b7280;margin-top:3px;line-height:1.6">${esc(w.description)}</p>`:''}</div><div style="font-size:13px;color:#9ca3af;text-align:right;white-space:nowrap">${esc(w.startDate)}<br>– ${esc(w.endDate||'Now')}</div></div>`).join('')}</div>`:''}
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
          ${(d.education||[]).length?`<div><div style="display:flex;align-items:center;gap:8px;margin-bottom:10px"><span style="font-size:12.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111827">Education</span><div style="flex:1;height:2px;background:#111827"></div></div>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><strong style="font-size:16px;color:#111827">${esc(e.degree)} in ${esc(e.field)}</strong><div style="font-size:14px;color:#6b7280">${esc(e.school)} · ${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
          ${(d.skills||[]).length?`<div><div style="display:flex;align-items:center;gap:8px;margin-bottom:10px"><span style="font-size:12.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111827">Skills</span><div style="flex:1;height:2px;background:#111827"></div></div><div style="display:flex;flex-wrap:wrap;gap:5px">${(d.skills||[]).map(s=>`<span style="font-size:14px;font-weight:800;color:#111827;border:1.5px solid #111827;padding:2px 10px;border-radius:3px">${esc(s)}</span>`).join('')}</div></div>`:''}
        </div>
    ...
    function tplExecutive(d,c,fc){
      ...
      return `<div class="${fc}" style="font-weight:500;font-size:17.5px;background:#fff;min-height:700px;padding:0">
        <!-- Header -->
        <div style="padding:28px 32px 20px;border-bottom:2px solid #111;display:flex;justify-content:space-between;align-items:flex-start;gap:20px">
          <div style="flex:1">
            <div style="font-size:13px;font-weight:800;letter-spacing:3px;text-transform:uppercase;color:#666;margin-bottom:6px">Curriculum Vitae</div>
            <h1 style="font-size:37.5px;font-weight:900;color:#111;letter-spacing:-0.5px;line-height:1.2;margin-bottom:4px;text-transform:uppercase">${esc(d.fullName||'Your Name')}</h1>
            <p style="font-size:16px;font-weight:800;color:${c};letter-spacing:2px;text-transform:uppercase;margin-bottom:10px">${esc(d.jobTitle||'')}</p>
            <div style="display:flex;flex-wrap:wrap;gap:12px;font-size:14px;color:#555">
        ...
        <div style="display:grid;grid-template-columns:1fr 180px;gap:0;min-height:560px">
          ...
          <div style="padding:20px 24px 20px 32px;border-right:1px solid #e5e7eb">
            ${d.summary?`<div style="margin-bottom:16px"><div style="font-size:12.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111;margin-bottom:6px;padding-bottom:4px;border-bottom:1.5px solid #111">Summary</div><p style="font-size:15px;color:#444;line-height:1.75">${esc(d.summary)}</p></div>`:''}
            ${(d.workExperience||[]).length?`<div style="margin-bottom:14px"><div style="font-size:12.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111;margin-bottom:8px;padding-bottom:4px;border-bottom:1.5px solid #111">Work Experience</div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16px;color:#111;text-transform:uppercase;letter-spacing:.3px">${esc(w.position)}</strong><div style="font-size:14.5px;color:${c};font-weight:700;margin-top:1px">${esc(w.company)}</div></div><span style="font-size:13px;color:#888;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Present')}</span></div>${w.description?`<p style="font-size:14.5px;color:#555;margin-top:3px;line-height:1.6">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
          </div>
          <!-- Right sidebar: skills + education -->
          <div style="padding:20px 20px 20px 16px;background:#f9fafb">
            ${(d.skills||[]).length?`<div style="margin-bottom:16px"><div style="font-size:11.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111;margin-bottom:8px;padding-bottom:4px;border-bottom:1.5px solid #111">Skills</div>${(d.skills||[]).map(s=>`<div style="font-size:14.5px;color:#374151;padding:3px 0;border-bottom:1px solid #e5e7eb">${esc(s)}</div>`).join('')}</div>`:``}
            ${(d.education||[]).length?`<div style="margin-bottom:16px"><div style="font-size:11.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111;margin-bottom:8px;padding-bottom:4px;border-bottom:1.5px solid #111">Education</div>${(d.education||[]).map(e=>`<div style="margin-bottom:8px"><strong style="font-size:14.5px;color:#111;display:block">${esc(e.degree)}</strong><div style="font-size:14px;color:#555">${esc(e.field)}</div><div style="font-size:13px;color:#7c3aed;font-weight:700">${esc(e.school)}</div><div style="font-size:13px;color:#888">${esc(e.graduationDate)}</div></div>`).join('')}</div>`:``}
    ...
    function tplNova(d,c,fc){
      return `<div class="${fc}" style="font-weight:500;font-size:17.5px;display:grid;grid-template-columns:200px 1fr;background:#fff;min-height:700px">
        <!-- Sidebar -->
        <div style="background:linear-gradient(180deg,${c} 0%,${c}dd 100%);color:#fff;padding:0;display:flex;flex-direction:column">
          ...
          <div style="padding:24px 20px 16px;text-align:center">
            ${d.photo
              ...
              : `<div style="width:80px;height:80px;border-radius:50%;background:rgba(255,255,255,.15);border:2px solid rgba(255,255,255,.3);display:flex;align-items:center;justify-content:center;font-size:43.5px;font-weight:900;margin:0 auto 12px">${(d.fullName||'?')[0].toUpperCase()}</div>`}
            <h1 style="font-size:20.5px;font-weight:800;line-height:1.35;margin-bottom:3px;color:#fff">${esc(d.fullName||'Your Name')}</h1>
            <p style="font-size:14.5px;font-weight:700;color:rgba(255,255,255,.75);letter-spacing:.5px">${esc(d.jobTitle||'')}</p>
          </div>
          <div style="height:1px;background:rgba(255,255,255,.15);margin:0 16px"></div>
          <!-- Contact -->
          <div style="padding:14px 20px;font-size:14px;color:rgba(255,255,255,.85);line-height:2">
          ...
          <div style="height:1px;background:rgba(255,255,255,.15);margin:0 16px"></div>
          <!-- Skills -->
          ${(d.skills||[]).length?`<div style="padding:14px 20px"><p style="font-size:11.5px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:rgba(255,255,255,.5);margin-bottom:8px">Skills</p><div style="display:flex;flex-direction:column;gap:5px">${(d.skills||[]).map(s=>`<div style="font-size:14.5px;color:rgba(255,255,255,.9);background:rgba(255,255,255,.12);border-radius:6px;padding:4px 10px;border-left:3px solid rgba(255,255,255,.4)">${esc(s)}</div>`).join('')}</div></div>`:''}
          <!-- Education -->
          ${(d.education||[]).length?`<div style="padding:10px 20px 20px"><p style="font-size:11.5px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:rgba(255,255,255,.5);margin-bottom:8px">Education</p>${(d.education||[]).map(e=>`<div style="margin-bottom:9px"><p style="font-size:14.5px;font-weight:800;color:#fff">${esc(e.degree)}</p><p style="font-size:13px;color:rgba(255,255,255,.7)">${esc(e.field)}</p><p style="font-size:13px;color:rgba(255,255,255,.5)">${esc(e.school)} · ${esc(e.graduationDate)}</p></div>`).join('')}</div>`:''}
          ${(d.languages||[]).length?`<div style="padding:0 20px 20px">${secLang(d,c,'dark')}</div>`:''}
        </div>
        <!-- Main content -->
        <div style="padding:28px 26px;background:#fff;line-height:1.65">
          ${d.summary?`<div style="margin-bottom:18px;padding:14px 16px;background:#f8f7ff;border-left:4px solid ${c};border-radius:0 8px 8px 0"><p style="font-size:16px;color:#374151;line-height:1.75;font-style:italic">${esc(d.summary)}</p></div>`:''}
          ${(d.workExperience||[]).length?`<div style="margin-bottom:16px"><div style="font-size:12.5px;font-weight:900;color:${c};text-transform:uppercase;letter-spacing:2px;margin-bottom:10px;display:flex;align-items:center;gap:8px">Experience <div style="flex:1;height:1.5px;background:${c}33"></div></div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid #f1f5f9"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16.5px;color:#111">${esc(w.position)}</strong><div style="font-size:14.5px;color:${c};font-weight:700;margin-top:1px">${esc(w.company)}</div></div><span style="font-size:13px;color:#94a3b8;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Present')}</span></div>${w.description?`<p style="font-size:14.5px;color:#475569;margin-top:4px;line-height:1.6">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
    ...
    function tplTribune(d,c,fc){
      return `<div class="${fc}" style="font-weight:500;font-size:17.5px;background:#fff;min-height:700px">
        <!-- Masthead -->
        <div style="background:#111;padding:22px 28px;display:flex;justify-content:space-between;align-items:center;gap:20px">
          <div style="flex:1">
            <div style="font-size:10px;font-weight:800;letter-spacing:4px;text-transform:uppercase;color:#9ca3af;margin-bottom:4px">Professional Resume</div>
            <h1 style="font-size:40.5px;font-weight:900;color:#fff;letter-spacing:-1px;line-height:1;text-transform:uppercase">${esc(d.fullName||'Your Name')}</h1>
            <div style="width:40px;height:3px;background:${c};margin:8px 0"></div>
            <p style="font-size:16px;font-weight:800;color:${c};letter-spacing:1.5px;text-transform:uppercase">${esc(d.jobTitle||'')}</p>
          </div>
        ...
        <!-- Contact strip -->
        <div style="background:${c};padding:7px 28px;display:flex;gap:20px;flex-wrap:wrap;font-size:14px;color:#fff;font-weight:700">
          ${d.email?`<span>✉ ${esc(d.email)}</span>`:''}
          ${d.phone?`<span>☎ ${esc(d.phone)}</span>`:''}
          ${d.location?`<span>⌖ ${esc(d.location)}</span>`:''}
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 175px;gap:0">
          ...
          <div style="padding:20px 20px 20px 28px;border-right:1px solid #e5e7eb">
            ${d.summary?`<div style="margin-bottom:16px;padding-bottom:14px;border-bottom:1px solid #e5e7eb"><p style="font-size:16px;color:#374151;line-height:1.8">${esc(d.summary)}</p></div>`:''}
            ${(d.workExperience||[]).length?`<div><div style="font-size:11.5px;font-weight:900;text-transform:uppercase;letter-spacing:2.5px;color:#111;margin-bottom:10px;display:flex;align-items:center;gap:8px">Experience<div style="flex:1;height:2px;background:#111"></div></div>${(d.workExperience||[]).map(w=>`<div style="margin-bottom:11px;padding-bottom:11px;border-bottom:1px solid #f1f5f9"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><strong style="font-size:16px;color:#111">${esc(w.position)}</strong><div style="font-size:14.5px;font-weight:700;color:${c}">${esc(w.company)}</div></div><span style="font-size:13px;color:#9ca3af;white-space:nowrap;margin-left:8px">${esc(w.startDate)} – ${esc(w.endDate||'Now')}</span></div>${w.description?`<p style="font-size:14.5px;color:#6b7280;margin-top:3px;line-height:1.6">${esc(w.description)}</p>`:''}</div>`).join('')}</div>`:''}
          </div>
          <!-- Right col -->
          <div style="padding:20px 20px 20px 16px;background:#fafafa">
            ${(d.skills||[]).length?`<div style="margin-bottom:16px"><div style="font-size:11.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111;margin-bottom:8px;display:flex;align-items:center;gap:6px">Skills<div style="flex:1;height:1.5px;background:#111"></div></div><div style="display:flex;flex-direction:column;gap:4px">${(d.skills||[]).map(s=>`<div style="font-size:14.5px;color:#374151;padding:3px 8px;background:#fff;border-radius:5px;border-left:3px solid ${c};font-weight:600">${esc(s)}</div>`).join('')}</div></div>`:''}
            ${(d.education||[]).length?`<div style="margin-bottom:14px"><div style="font-size:11.5px;font-weight:900;text-transform:uppercase;letter-spacing:2px;color:#111;margin-bottom:8px;display:flex;align-items:center;gap:6px">Education<div style="flex:1;height:1.5px;background:#111"></div></div>${(d.education||[]).map(e=>`<div style="margin-bottom:9px"><strong style="font-size:14.5px;color:#111;display:block">${esc(e.degree)}</strong><div style="font-size:14px;color:#555">${esc(e.field)}</div><div style="font-size:13px;color:${c};font-weight:700">${esc(e.school)}</div><div style="font-size:13px;color:#9ca3af">${esc(e.graduationDate)}</div></div>`).join('')}</div>`:''}
            ${secCert(d,c)}${secLang(d,c)}
          </div>
        </div>
          ...
          <div class="km-ai-body">
            ...
            <div class="km-actions" id="kact-${msgId}">
              <button class="km-act-btn" onclick="kieyCopy(this)" title="Copy response">
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
              </button>
              <button class="km-act-btn" onclick="kieSpeak(this)" title="Listen aloud">
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path stroke-linecap="round" d="M19.07 4.93a10 10 0 010 14.14M15.54 8.46a5 5 0 010 7.07"/></svg>
              </button>
    ...
    function _ensureKieFilePreviewOverlay() {
      ...
      ol.innerHTML = `
        <div class="kie-file-sheet">
          <div class="kie-file-handle"></div>
          <div class="kie-file-overlay-hdr">
            ...
            <button class="kie-file-overlay-close" onclick="closeKieFilePreview()">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M6 6l12 12M6 18L18 6"/></svg>
            </button>
          </div>
...
Return ONLY valid JSON, no markdown, no explanation.`;
    ...
    async function runAnalysis() {
      ...
      try {
        ...
        scan.finish();
        document.querySelectorAll('#uploadFlowDots .kie-flow-dot').forEach((d, i) => { d.classList.toggle('active', i === 1); d.classList.toggle('done', i === 0); });
        showView('analysis');
      } catch (err) {
        if (scan) scan.finish();
