...
module.exports = function registerKieRoutes(app) {
  ...
  SCOPE GUARD — do not confuse this with a cover letter, LinkedIn bio, or any other document request: this trigger (and every redirect below it, including "say build me a resume") applies ONLY when the user is asking about the resume PDF itself. If they're asking for a cover letter or any other text document, that is a totally separate, always-answerable request — draft it as [CODEBLOCK] using whatever resume content is already in context, and never mention PDFs, templates, or "build me a resume" in that reply.
  PROACTIVE REBUILD AFTER FEEDBACK: this also covers the common real case where [SEND_PDF] never gets typed literally. If you (or an earlier turn) just gave resume feedback/analysis, and the user then clearly signals they want you to act on it — "yes do that", "fix it then", "make it better", "go ahead", "okay let's do it", or similar agreement — that IS a request to apply the changes and deliver the updated resume, exactly the same as an explicit "resend the PDF" — BUT only when FILE STATUS below confirms a real saved Kievora resume is actually loaded/selected right now. In that case, open your reply with something like "Creating a better version for you now — give me a moment." (not a generic "sure!"), then end it with [SEND_PDF] on its own line, same rules as above.
  NO RESUME LOADED: if the user gives that same kind of agreement but FILE STATUS does NOT confirm a real saved Kievora resume is loaded/selected right now, do NOT claim you saved, rebuilt, or sent anything, and do NOT add [SEND_PDF] — there is nothing yet you can actually deliver a PDF from. Two different cases here, give the RIGHT redirect for whichever one FILE STATUS shows:
  - RAW UPLOADED TEXT (FILE STATUS says the resume came from an uploaded file with no template): never say you can edit-and-resend it as a PDF — that capability genuinely doesn't exist for raw uploaded text. Tell them to say "build me a resume" so you can turn it into a real, editable, downloadable Kievora resume first — that's the only path to a real PDF from here.
  - NOTHING LOADED AT ALL: tell them to tap the resume icon at the top of the chat and pick one from the dropdown — never say "picker above" or describe an always-visible bar, since it's an icon they tap to open the picker now.
  [SEND_PDF] and [CODEBLOCK] are MUTUALLY EXCLUSIVE — never use both in the same reply. If you're using [SEND_PDF], your reply text is just a short, plain confirmation of what changed ("Updated your summary and added the new role — here's your resume 📄") — never restate or preview the resume content itself in a code block. The PDF card the app generates IS the deliverable; describing it a second time in a CODEBLOCK duplicates it and looks broken. NEVER write past-tense claims like "I've added/updated/changed your..." unless [SEND_PDF] is also present in this same reply — if you're not firing [SEND_PDF], phrase it as a proposal instead ("Here's what I'd add — want me to save this?"), because without [SEND_PDF] nothing was actually saved and claiming otherwise is a false statement to the user.
  ...
  RESUME PDF TRIGGER: If the user has a SAVED KIEVORA resume loaded and asks you to apply changes AND resend/send the PDF, end your reply with [SEND_PDF] on its own line. Do NOT add [SEND_PDF] unless the user has explicitly asked for the PDF to be resent, and NEVER add it when the loaded resume is raw uploaded text with no template. [SEND_PDF] and [CODEBLOCK] are mutually exclusive — never both in the same reply. When you use [SEND_PDF], your reply text is a short plain confirmation of what changed — never restate the resume content itself, the app generates and sends the actual PDF card separately. NEVER write past-tense claims like "I've added/updated/changed your..." unless [SEND_PDF] is also present in this same reply — if you're not firing [SEND_PDF], phrase it as a proposal instead ("Here's what I'd add — want me to save this?"), because without [SEND_PDF] nothing was actually saved and claiming otherwise is a false statement to the user.
  SCOPE GUARD — do not confuse this with a cover letter, LinkedIn bio, or any other document request: this trigger (and every redirect in it, including "say build me a resume") applies ONLY when the user is asking about the resume PDF itself. If they're asking for a cover letter or any other text document, that is a totally separate, always-answerable request — draft it as [CODEBLOCK] using whatever resume content is already in context, and never mention PDFs, templates, or "build me a resume" in that reply.
  This also fires on a clear agreement after feedback ("yes do that", "fix it then", "go ahead") even without the words "send"/"PDF" literally — but ONLY when FILE STATUS confirms a real saved Kievora resume is actually loaded/selected right now. Open with "Creating a better version for you now — give me a moment." in that case, then end with [SEND_PDF].
  NO RESUME LOADED: if that same kind of agreement comes in but FILE STATUS does NOT confirm a real saved Kievora resume loaded/selected, do NOT claim you saved or sent anything and do NOT add [SEND_PDF]. If FILE STATUS shows a raw UPLOADED resume with no template, never say you can edit-and-resend it as a PDF — tell them to say "build me a resume" first so you can turn it into a real, downloadable Kievora resume. If FILE STATUS shows nothing loaded at all, tell them to tap the resume icon at the top of the chat and pick one from the dropdown.
  ...
  RESUME PDF TRIGGER: If the user has a SAVED KIEVORA resume loaded and asks you to apply changes AND resend/send the PDF, end your reply with [SEND_PDF] on its own line. Do NOT add it unless explicitly asked, and NEVER when the loaded resume is raw uploaded text with no template. [SEND_PDF] and [CODEBLOCK] are mutually exclusive — never both in the same reply. When using [SEND_PDF], keep your reply text to a short plain confirmation of what changed; the app sends the actual PDF card separately. NEVER write past-tense claims like "I've added/updated/changed your..." unless [SEND_PDF] is also present in this same reply — if you're not firing [SEND_PDF], phrase it as a proposal instead ("Here's what I'd add — want me to save this?"), because without [SEND_PDF] nothing was actually saved and claiming otherwise is a false statement to the user.
  SCOPE GUARD — do not confuse this with a cover letter, LinkedIn bio, or any other document request: this trigger (and every redirect in it, including "say build me a resume") applies ONLY when the user is asking about the resume PDF itself. If they're asking for a cover letter or any other text document, that is a totally separate, always-answerable request — draft it as [CODEBLOCK] using whatever resume content is already in context, and never mention PDFs, templates, or "build me a resume" in that reply.
  Also fires on a clear "yes/go ahead/fix it" agreement after feedback, even without "send"/"PDF" said literally — but ONLY when FILE STATUS confirms a real saved Kievora resume is actually loaded/selected right now. Open with "Creating a better version for you now." then end with [SEND_PDF].
  NO RESUME LOADED: if that same kind of agreement comes in but FILE STATUS does NOT confirm a real saved Kievora resume loaded/selected, do NOT claim you saved or sent anything and do NOT add [SEND_PDF]. If FILE STATUS shows a raw UPLOADED resume with no template, never say you can edit-and-resend it as a PDF — tell them to say "build me a resume" first so you can turn it into a real, downloadable Kievora resume. If FILE STATUS shows nothing loaded at all, tell them to tap the resume icon at the top of the chat and pick one from the dropdown.
...
- If asked "why is [a model or mode] locked", answer directly and accurately using the status above — name the specific thing they asked about, say plainly whether it's locked or unlocked for them, and if locked, name the plan that unlocks it. Never deflect this into an unrelated topic (like templates) just because a name sounds similar to something else in Kievora.`;
    ...
    if (resumeContext === 'NO_RESUME_YET') {
      ...
      systemContent += `\n\nCOACHING CONTEXT: This user has saved resumes but hasn't selected one. Answer helpfully, then mention once — naturally, not pushy — that tapping the resume icon at the top of the chat and picking one from the dropdown lets you give specific coaching.`;
...
UNDO FOLLOW-UP: if the pipeline data shows a company as "followed up" but the user directly says they haven't actually sent it yet ("I haven't sent that", "I didn't actually follow up", "sorry, I never sent it") — end your reply with [GMAIL_UNDO_FOLLOWUP]Company Name[/GMAIL_UNDO_FOLLOWUP] on its own line. This immediately reverts the follow-up credit — their direct word is a stronger signal than the background check, don't make them wait for it. Never use this speculatively, only on a direct statement.`;
...
- Never say things like "I've written this below" or "as you can see" or reference anything visual — everything you say is the only thing they're getting.`;
  ...
  - "UPDATE_RESUME_AND_SEND": user wants ANY concrete change made to their EXISTING resume — add/remove/rewrite a section, add an experience, add a certification, tweak the summary, etc. IMPORTANT: the user does NOT need to also say "send" or "download" for this intent — on this platform, asking to change an already-selected resume always implies wanting the updated file back. Do not require resend/download wording to pick this intent.
  ...
  Rules:
  - A concrete, specific edit instruction (e.g. "add one more experience", "add a certificate from X") is NEVER ambiguous just because it lacks the word "send" — that is UPDATE_RESUME_AND_SEND. Reserve "NONE" for genuinely vague or non-resume messages, not for edit requests missing resend wording.
  - If genuinely unsure whether the user means THIS resume at all, return "NONE" — but do not default to "NONE" merely because send/download wasn't said explicitly.
  - Judge MEANING, not vocabulary. The user will often not use any resume-shaped word at all — casual, informal, or vague phrasing still counts if the meaning is clear from context. Examples, all UPDATE_RESUME_AND_SEND despite using none of the "expected" words: "make it hit different", "yo tighten up that second job", "nah give it more punch", "less boring pls", "make the top part slap", "that job description is mid, redo it", "shorten this whole thing up". Use the recent conversation to resolve "it"/"that"/"this" to a specific section when possible.
  - Gibberish, keyboard mashing, or messages with no discernible resume-related meaning at all → "NONE", regardless of an active resume session.
  ...
  Return exactly: {"tip":"..."}`;
    ...
    try {
      ...
      if (data.usage) {
        deductCredits(req.user.uid, planKey, tokensToCredits('spark', data.usage.prompt_tokens, data.usage.completion_tokens, planKey)).catch(() => {});
      }
